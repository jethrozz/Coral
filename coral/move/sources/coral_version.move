module coral::coral_version{
    use sui::vec_set::{Self, VecSet};
    
        // 权限
    public struct CoralVersionAdminCap has key, store {
        id: UID
    }
    ...
    fun init(ctx: &mut TxContext) {
        //创建管理员权限
        let admin = CoralVersionAdminCap { id: object::new(ctx) };
    ...
    public fun add_version(_cap: &CoralVersionAdminCap, config: &mut GlobalConfig, version: u16) {
        config.version.insert(version);
    }

    public fun remove_version(_cap: &CoralVersionAdminCap, config: &mut GlobalConfig, version: u16) {
