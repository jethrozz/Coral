module coral::coral_market {
    use sui::sui::SUI;
    use sui::{package, display};
    use sui::{clock::Clock, coin::{Self, Coin}, table::{Self, Table}, balance::{Self,Balance}};
    use coral::coral_version::{Self, GlobalConfig};
    use coral::coral_event::{Self};
    use std::string::String;
    use coral::coral_sync::{Self, File};
    //彩票的一次性见证者
    public struct CORAL_MARKET has drop {}
    // 权限
    public struct CoralAdminCap has key, store {
        id: UID
    }
    ...
    public struct Installment has key {
        ...
        no: u64,
        files: vector<ID>,
        is_published: bool, //是否已发布，只有发布后订阅者才能访问
    ...
    */
    ...
    const E_INSTALLMENT_NOT_PUBLISHED: u64 = 1013;
    const E_INSTALLMENT_ALREADY_PUBLISHED: u64 = 1014;
    const E_FILE_NOT_IN_INSTALLMENT: u64 = 1015;
    fun init(otw: CORAL_MARKET, ctx: &mut TxContext) {
        ...
        //创建nft
            ...
            let values = vector[
                ...
                b"https://coral.walrus.site".to_string(),
    ...
    public entry fun create_column(
        ...
        let col_cap = ColumnCap{
            ...
            name,
            description: desc,
            link: b"https://coral.walrus.site".to_string(),
    ...
    public fun edit_column_base_info(
        ...
        ctx: &mut TxContext){
            ...
            if(column.status == 1){
                coral_event::update_column_event(ctx.sender(), object::id(column), column.name, old_name, column.desc, old_desc, column.is_rated);
            }
    }
    ...
    public entry fun publish_column(
        ...
        _ctx: &mut TxContext){
            ...
            column.updated_at = clock.timestamp_ms();
        //专栏发布事件
        coral_event::publish_column_event(_ctx.sender(), object::id(column), column.name, column.payment_method, column.update_method, column.is_rated)
    }
    ...
    // 发布installment，只有发布后订阅者才能访问
    public entry fun publish_installment(
        column_cap: &ColumnCap,
        column: &mut Column,
        installment: &mut Installment,
        clock: &Clock,
        ctx: &mut TxContext){
            assert!(object::id(column) == column_cap.column_id, E_NO_COLUMN_CAP);
            assert!(installment.belong_column == object::id(column), E_NO_COLUMN_CAP);
            assert!(!installment.is_published, E_INSTALLMENT_ALREADY_PUBLISHED);
            
            let now = clock.timestamp_ms();
            installment.is_published = true;
            installment.published_at = now;
            
            //专栏每期更新事件
            coral_event::installment_publish_event(ctx.sender(), object::id(column), object::id(installment), installment.no);
    }
    ...
    public entry fun subscription_column(
        ...
        ctx: &mut TxContext){
            ...
            coral_event::subscribe_column_event(ctx.sender(), object::id(column), now, ent_time, column.payment_method, sub_fee_value, cut_fee_value, payment_method.coin_type);
    }
    ...
    public entry fun renew_subscription(
        ...
        ctx: &mut TxContext){
            ...
            //续费事件
            let ent_time = now + payment_method.subscription_time;
            coral_event::renew_column_event(ctx.sender(), object::id(column), now, ent_time, column.payment_method, sub_fee_value, cut_fee_value, payment_method.coin_type);
    }
    ...
    entry fun seal_approve_creator(_id: vector<u8>, col_cap: &ColumnCap, column: &Column, file: &File, _ctx: &TxContext) {
        // 检查是否是该专栏的作者
